...
def main():
    ...
    plt_rect_rgns = alt_rgns.encode(
        x="xstart:Q", x2="xend:Q", y="ystart:Q", y2="yend:Q", color=regcolor
    )
    ...
    plt_kmeans = (
        alt.Chart(df).mark_point().encode(x="rho:Q", y="theta:Q", color=allcolor)
    )
    ...
    mypp.close()
    alt.vconcat(*alt_graphs).resolve_scale(color="independent").save(
        "charts_kmeans.html"
    )
...
def kmeans_it(X):
    ...
    kneedle = kneed.KneeLocator(
        range(my_sods.size), my_sods, curve="convex", direction="decreasing"
    )
    kneedle_delta = 2
