...
with h5py.File("file.hdf5", "w") as hf:
    ...
    dset = hf.create_dataset("my_data", (100,), dtype="i", compression="gzip")
    dset[...] = np.arange(100)
    ...
    seg_ds = grp.create_dataset("train", data=train, compression="gzip")

    # we can easily create nested groups
    sub_grp = grp.create_group("subgroup1/subgroup11")
    ones_arr = np.ones((250, 5000))
    sub_ds = sub_grp.create_dataset("sensors", data=ones_arr, compression="gzip")
